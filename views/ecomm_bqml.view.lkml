    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `bi-eng-internal.ecommerce_sample.order_items` as oi
                  JOIN `bi-eng-internal.ecommerce_sample.inventory_items` as inv ON inv.id = oi.inventory_item_id
      ...
      JOIN (SELECT user_id as user_id1
            ...
            FROM `bi-eng-internal.ecommerce_sample.events`
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `bi-eng-internal.ecommerce_sample.kmeans_model5`,
        (SELECT * FROM
